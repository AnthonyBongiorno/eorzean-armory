...
const verifyCharacter = async (req, res) => {
  try {
    // Extract the character first name, last name, and server from the request query parameters
    const { firstName, lastName, serverName } = req.query;

    // Find the character with the provided first name, last name, and server
    const resData = await xiv.character.search(`${firstName} ${lastName}`, serverName);
    ...
    const isClaimed = await db.collection('claimedID').findOne({ characterName: characterName });
